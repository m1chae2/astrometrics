...
| **2. Public Helpers**<br>*(calls L3)* | `stack_and_solve`<br>*(pre-stage)* | `analyze_target`<br>*(runner)* | `analyze_target`<br>*(runner)* | `analyze_target`<br>*(runner)* | `analyze_target`<br>*(runner)* | **orchestration/**<br>`analysis_router`<br>`stack_and_solve`<br>`full_target_pipeline`<br>`pipeline_base`<br>`runners` |
...
| **4. Driver Access**<br>*(exposed via L1)* | *(handed one by stack_and_solve)* | `catalog_access` | `catalog_access` | `catalog_access` | *(skips L4/L5)* | **driver_access/**<br>`catalog_access`<br>`frame_scanning` |
...
- **Dispatch and orchestration:** `astrometricslib/pipelines/analysis_router.py` (picks which analysis pipeline runs, by name), `astrometricslib/pipelines/stack_and_solve.py` (stacks a target's frames, then chains an astrometry solve onto them), `astrometricslib/pipelines/full_target_pipeline.py` (runs a target's full stack-solve-photometry-spectroscopy sequence), and `astrometricslib/api/batch.py` (runs many targets through that sequence in parallel)
