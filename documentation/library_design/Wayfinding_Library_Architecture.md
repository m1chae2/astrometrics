...
This paper presents an architecture for the observatory-side half of the Astrometrics software suite, whose purpose is to operate a single-observer observatory unattended, and in doing so to replace the third-party capture and guiding software the observatory currently depends on. It is organized around three functions: **Observatory Control**, the direct operation of telescope, camera, enclosure, and mount hardware together with the safety monitoring that protects them; **Observation Planning**, the construction of reusable imaging requests and their arrangement into a feasible, diagnosable night; and **Observation Execution**, the optional function that carries a planned night out, recording what happened alongside what was intended. Control and Planning are independently usable; Execution is built on top of both and depends on neither depending on it. Cutting across all three is a per-capability delegation model: each hardware-facing capability independently occupies one of three states — delegated to the incumbent software, shadowed against it, or authoritative — so that responsibility transfers one capability at a time, on recorded evidence, across six named phases ending with the incumbent removed. The architecture also establishes a durable record of each night's operational context, linked by identifier to the science-side quality records produced by image processing.

## 1. Introduction

**Statement of need.** An observing night involves three separable decisions that existing amateur tooling conflates or omits. Deciding *what* to image and in *what order* is left to operator memory and ad hoc note-taking; there is no reusable record of "twenty subs of M 81 in luminance, stop below thirty degrees." Deciding *how* to physically operate the equipment is handled well by device drivers, but only as isolated commands with no relationship to a plan. And carrying a night out — running through several targets, waiting for the next one to rise, adapting when conditions change — happens under supervision, with no record of what was intended against what actually occurred. Existing observatory software computes rich operational telemetry but does not preserve it in a form that can be joined to a plan or to the image-quality outcomes that plan produced.

**The destination is replacement, and the constraint is that the observatory must work throughout.** The incumbent capture, focus, guiding, and alignment software is trustworthy precisely because it is proven, and the goal of this architecture is to take over every function it performs and remove it. Replacing it in a single cutover would discard that proof in one step with no evidence the replacement is equivalent, on a system whose failures damage equipment. What is needed is a structure in which each capability is independently computed alongside the incumbent, compared against it on real nights, and promoted only once the comparison supports promotion — with the observatory fully operational at every intermediate configuration, and with the incumbent's removal as the defined endpoint rather than an option left open.

**Unattended operation is the target, which makes safety architectural rather than incidental.** A system that runs dusk to dawn with nobody present must be able to recognize that conditions or its own hardware have gone wrong, and reach a physically safe state on its own. That requirement does not sit inside the observing loop; it must be able to interrupt it, and it must work when no observing loop is running at all.

This document describes an architecture that separates these concerns so each can be built, used, and verified independently, and that makes the delegation boundary an explicit, per-capability, evidence-gated property. Observation Planning and Observatory Control are peer functions: an operator can plan a night's targets without ever commanding hardware, and the observatory can protect itself with no plan in existence. Observation Execution sits above both and is the only function whose absence changes nothing below it.

Furthermore, this architecture adheres to the same dual-interface paradigm as the science-side processing system. The programmatic interfaces that drive these functions are identical to those invoked by any graphical client. Observers can interactively direct the observatory via a user interface, and directly encode those identical operational maneuvers into programmatic scripts for complete automation, with no hidden logic intermediate between the operator and the system.

Section 2 presents the architecture, including the software realization, module mappings to the Sphinx API reference, and commissioning metrics. Section 3 discusses the design assumptions it revises. Section 4 concludes.
