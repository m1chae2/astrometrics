...
**Statement of need.** Transforming raw astronomical exposures into science-grade data requires rigorous calibration to remove instrumental signatures and sophisticated algorithms to extract precise measurements. This guide details the step-by-step workflow for leveraging the Astrometrics processing pipelines to produce high-quality stacked images and extract meaningful scientific data (photometry and spectroscopy).
...
1. Switch to **Image Processing** mode (`Ctrl + 2`).
2. Select your target name from the **Observation Targets** list in the left sidebar.
3. Under **Calibration Assets**, select the matching master calibration frames that correspond to your camera's temperature, gain, and optical train:
   - **Master Bias**: Subtracts sensor read noise. Ensure the bias frame matches the camera gain.
   - **Master Dark**: Subtracts thermal dark current (heat noise). Ensure the dark frame matches the exposure time and sensor temperature of your light frames.
...
*(For exact arithmetic operations applied during calibration, refer to the {py:class}`~astrometricslib.api.processing.ProcessingPipelines` API Reference)*
...
2. Drag the **FWHM Cutoff** slider to reject sub-exposures where star FWHM exceeds your target threshold (e.g., $>3.5''$). This ensures only the sharpest frames contribute to the final stack.
...
1. Select a **Reference Frame**. Choose a frame from the middle of your session with a high star count, low FWHM, and high roundness.
...
1. Switch to **Astronomy Manager** mode (`Ctrl + 3`).
2. Load your stacked sequence and select your target variable star or exoplanet host star.
...
*(For differential magnitude calculus and BLS algorithm parameters, see the {py:class}`~astrometricslib.api.stars.StellarCatalog` API Reference)*
...
[1] M. Coniglio, "Scientific Image Processing and Photometric Calibration Handbook," 2026.
